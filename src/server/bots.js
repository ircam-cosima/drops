...
class Bot {
  constructor(index, uuid, coords, experience) {
    ...
    const startDelay = (4 + 30 * Math.random()) * 1000;
  ...
  _start() {
    ...
    exp.salesman.addPoi(this.uuid, this.coordinates);
    exp.uuidClientMap.set(this.uuid, this);
  ...
  destroy(now = false) {
    const destroyDelay = now ? 0 : (4 + 30 * Math.random()) * 1000;
